...
4. 🐣 **실시간으로 부화.** 커먼은 30분, 레어는 2시간, 에픽은 6시간, 레전더리는 24시간 — 앱을 꺼 둬도 흐르는 실제 시계 기준입니다. 알은 [PokéAPI](https://pokeapi.co/)의 실제 진화 데이터를 가진 진화 전(베이스) 종만 낳고, 25종 성격 중 하나가 정해지며 — 아주 드문 우연으론 **✨ 이로치**로 태어납니다.
5. ⚡ **직접 진화시키세요.** 코딩(또는 상점의 경험치 사탕)으로 경험치를 채우고, 준비되면 눌러서 진화시키세요 — 분기가 있는 라인은 직접 갈 길을 고를 수 있습니다.
6. 📖 **두 가지 컬렉션을 채우세요.** **도감**은 지금까지 부화시킨 모든 종을 1번부터 1025번까지 기록합니다(못 본 종은 실루엣). **박스**는 보유한 모든 개체를 담습니다 — 중복은 자연스러운 일이고, 개체마다 각자의 경험치와 진화 상태를 가져서 코이킹과 갸라도스를 동시에 가질 수도 있어요.
...
<b>도감</b>은 1번부터 1025번까지 종을 체크하는 목록입니다 — 부화시키기 전엔 실루엣이에요. <b>박스</b>는 부화하거나 진화시킨 모든 개체를 담습니다 — 각자 등급·성격·획득일·경험치를 따로 가지고, 이로치는 ✨ 배지를 답니다. 중복은 완전히 자연스러운 일이에요.
</td>
...
<td width="55%" valign="middle">
<h3>🛒 경제를 위한 상점</h3>
그동안 쓴 토큰이 곧 재화입니다 — 확률이 버튼에 바로 표시된 가격으로 알을 뽑고, 부화 슬롯을 3개에서 6개까지 늘리고, <b>경험치 사탕</b>으로 포켓몬을 키우거나 <b>반짝이는 사탕</b>으로 바로 이로치를 만들거나, 부화 이로치 확률을 1/64에서 1/48로 영구히 올리는 <b>이로치 부적</b>을 살 수 있어요.
</td>
</tr>
</table>
