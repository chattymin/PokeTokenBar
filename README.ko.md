...
<tr>
<td width="45%" align="center"><img src="assets/screenshot-scan-folders-ko.png" width="300" alt="설정 → 고급의 원격 Mac"></td>
<td width="55%" valign="middle">
<h3>💻 맥이 두 대라면 토큰을 합산</h3>
<b>설정 → 고급</b>에서 SSH 대상으로 원격 Mac을 추가하면 그 Mac의 로컬 에이전트 로그(Pi, omp, Claude Code, Codex 등)를 <code>rsync</code>/<code>ssh</code> 로 읽어 합산 총계에 반영합니다 — 여러 Mac의 코딩을 한 마리 companion 이 먹습니다. 읽기 전용: 원격에는 절대 쓰거나 삭제하지 않고, 기존 SSH 자격증명으로 가져오며, 원격이 잠깐 연결이 안 되면 마지막으로 가져온 스냅샷을 유지하고 설정에 상태를 표시합니다.
</td>
</tr>
<tr>
<td width="55%" valign="middle">
...
- **원격 Mac** — SSH 로 다른 Mac의 사용량 로그를 읽어 합산 총계에 반영; 읽기 전용, 원격에 쓰지 않음 (설정 → 고급).
...
다른 Mac의 사용량도 합칠 수 있습니다: **설정 → 고급 → 원격 Mac** 에 SSH 대상을 추가하면, 그 머신의 프로바이더 로그 루트만 머신·프로바이더별 폴더(Application Support 아래)로 미러링한 뒤 기존 리더와 완전히 같은 방식으로 파싱합니다. 읽기 전용이라 rsync 는 `--delete` 없이 읽기 모드로만 돌고, 원격 파일은 절대 쓰기·수정·삭제하지 않습니다. 원격 실패 시 마지막으로 가져온 스냅샷을 유지하고 상태를 표시합니다. 가져오기는 기존 SSH 설정·키를 그대로 씁니다(배치 모드, 비밀번호 프롬프트 없음).
