...
1. 🥚 **평소처럼 코딩하세요.** 지원되는 코딩 도구에서 태우는 토큰이 알을 품습니다 — 따로 돌릴 건 없어요.
...
- **서비스별 탭** — 감지된 코딩 도구가 2개 이상이면 작은 탭으로 상세를 서비스별 전환(오늘 합계는 통합 유지).
...
macOS 14+ (Apple Silicon 또는 Intel). 끝입니다 — 토큰 사용량은 로컬 데이터에서 직접 읽으며 외부 CLI가 필요 없습니다.
