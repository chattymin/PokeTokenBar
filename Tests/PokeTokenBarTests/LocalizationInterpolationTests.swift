...
/// (같은 공백을 프랑스어 PR #185 도 지적한다. 둘 중 하나가 먼저 머지되면 다른 쪽은 이 파일을 버리면 된다.)
///
...
final class LocalizationInterpolationTests: XCTestCase {
    ...
    private func expect(_ lang: AppLanguage, _ member: String, _ produced: String,
        ...
        for needle in needles {
            XCTAssertTrue(produced.contains(needle),
                          "\(lang.rawValue).\(member): '\(needle)' 누락 → '\(produced)'",
                          file: file, line: line)
        }
    }
    ...
    func testInterpolationsSurviveInEverySupportedLanguage() {
        ...
        for lang in AppLanguage.allCases {
            ...
            expect(lang, "codexWindow(h)", l.codexWindow(420), "7")      // 420분 → 7시간
            ...
            expect(lang, "intervalLabel", l.intervalLabel(1860), "31")   // 1860초 → 31분
    ...
    func testGuardRejectsATranslationThatDroppedItsPlaceholder() {
        ...
        let dropped = "Hoje: tokens"                     // \(tokens) 를 흘린 번역
        XCTAssertTrue(intact.contains(Self.a))
        XCTAssertFalse(dropped.contains(Self.a), "센티널이 빠진 문구를 통과시키면 가드가 무의미하다")
    }
}
