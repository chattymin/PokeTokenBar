...
1. 🥚 **いつも通りコーディング。** 対応しているコーディングツールで使うトークンがタマゴを温めます — 追加の操作は不要です。
...
- **サービス別タブ** — 検出されたコーディングツールが2つ以上あると、小さなタブでサービス別の詳細を切替（今日の合計は合算のまま）。
...
macOS 14+（Apple Silicon または Intel）。それだけ — トークン使用量はローカルデータから直接読み取り、外部 CLI は不要です。
