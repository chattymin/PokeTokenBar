...
<td width="55%" valign="middle">
<h3>設定はお好みで</h3>
メニューバー表示項目、更新間隔（1–15分／手動）、ログイン時に起動、上限セクションだけを隠す Keychain オフ、警告／危険の閾値つき上限通知、パートナーのイベント通知。<b>韓国語／英語／日本語／スペイン語／フランス語／ポルトガル語／ドイツ語</b>の UI とポケモン名を完備。
</td>
</tr>
<tr>
<td width="55%" valign="middle">
...
<tr>
<td width="45%" align="center"><img src="assets/screenshot-scan-folders-ja.png" width="300" alt="設定 → 詳細のリモートMac"></td>
<td width="55%" valign="middle">
<h3>💻 Mac が2台ならトークンを合算</h3>
<b>設定 → 詳細</b> で SSH ターゲットを指定してリモート Mac を追加すると、その Mac のローカルエージェントログ（Pi・omp・Claude Code・Codex など）を <code>rsync</code>/<code>ssh</code> で読み取り、合算トータルに反映します — 複数の Mac でのコーディングを1匹のコンパニオンが育ちます。読み取り専用: リモートのファイルを書き換え・削除することはなく、既存の SSH 資格情報で取得し、接続できない間は最後に取得したスナップショットを保持して設定に状態を表示します。
</td>
</tr>
<tr>
<td width="55%" valign="middle">
...
- **リモートMac** — SSH で別の Mac の使用ログを読み取り合算トータルに反映; 読み取り専用、リモートには書き込まない（設定 → 詳細）。
...
別の Mac の使用量も合算できます: **設定 → 詳細 → リモートMac** に SSH ターゲットを追加すると、そのマシンのプロバイダーログルートだけをマシン・プロバイダー別フォルダ（Application Support 配下）にミラーしてから、既存のリーダーとまったく同じ方法で解析します。読み取り専用のため rsync は `--delete` なしの読み取りモードでしか動かず、リモートのファイルは書き込み・変更・削除されません。リモート失敗時は最後に取得したスナップショットを保持し、状態を表示します。取得は既存の SSH 設定・鍵をそのまま使います（バッチモード、パスワードプロンプトなし）。
