...
enum WindowsTray {
    ...
    private static func drawItemCard(_ hdc: HDC?, top: Int32, _ e: ShopCardEntry) {
        ...
        // Icon (real item sprite) or emoji fallback.
    ...
    /// Auto-update: download the release zip (via `gh`, which handles private-repo auth + redirects),
    /// extract it, and hand off to a detached updater script that waits for this app to exit, replaces
    /// the portable install folder in place, and relaunches. Falls back to opening the release page
    /// (manual download) if any step fails — e.g. `gh` not installed or a non-writable install dir.
    ...
    private static func performUpdate(_ upd: WindowsUpdate.Available) -> Bool {
        ...
        // Download the installer (gh does the private-repo auth + asset redirect).
        guard runToCompletion("gh release download win-\(upd.version) --repo \(WindowsUpdate.repo) --pattern \"*Setup*.exe\" --dir \"\(dl.path)\" --clobber"),
...
extension CompanionStore {
    ...
    var windowsDisplay: CompanionDisplay {
        ...
        func shopCard(_ entry: ShopEntry) -> ShopCardEntry {
            ...
            case .freshEgg:
                return ShopCardEntry(
                    icon: nil, emoji: "🥚", name: loc.freshEggName, desc: loc.freshEggDescription,
