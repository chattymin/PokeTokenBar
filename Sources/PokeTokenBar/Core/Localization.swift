...
struct L {
    ...
    var dexOpenDetailHint: String {
        t("탭하면 도감 설명", "Tap for Pokédex entries", "タップで図鑑説明", "Toca para ver las descripciones")
    }
    var dexBack: String { t("도감으로 돌아가기", "Back to Pokédex", "図鑑に戻る", "Volver a la Pokédex") }
    ...
    var dexFlavorLoading: String {
        t("도감 설명 불러오는 중…", "Loading Pokédex entries…", "図鑑説明を読み込み中…",
          "Cargando descripciones de la Pokédex…")
    }
    ...
    var dexFlavorEmpty: String {
        t("이 언어로 제공되는 도감 설명이 없어요", "No Pokédex entries in this language",
          "この言語の図鑑説明はありません", "No hay descripciones de la Pokédex en este idioma")
    }
