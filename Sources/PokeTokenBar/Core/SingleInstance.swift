...
/// 그 종료는 정상 종료(exit 0)라 `KeepAlive.SuccessfulExit=false` 인 에이전트가 되살리지 않고
/// (`UpdateChecker.applyUpdate` 가 이미 같은 성질에 기대고 있다), 다음 로그인부터는 에이전트가 단독으로
/// 실행하며 크래시 워치독도 정상 동작한다.
enum SingleInstance {
