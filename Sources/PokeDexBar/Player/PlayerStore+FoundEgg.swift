...
///
/// **한 번에 한 개다.** 경험치는 알 임계에서 멈추고(`PlayerStore.update` 의 상한), 받으면
/// 0 으로 돌아간다. 그래서 알이 두 개 예약되는 일이 없다 — 받아야 다시 쌓인다.
///
/// 그 대신 **다 찬 채로 두면 그동안의 경험치는 안 쌓인다.** 알을 받는 것이 밀린 수령이 아니라
/// 결정이 되게 하려는 의도된 맞바꿈이다(재화·사탕은 상한과 무관하게 계속 들어온다).
extension PlayerStore {
    ...
    func takeFoundEgg(individualID: UUID, line: EvoLine) -> Egg? {
        ...
        mutate { s in
            ...
            // **0 으로 되돌린다**(진화의 이월과 다르다). 경험치가 애초에 알 임계에서 멈추므로
            // 버릴 초과분이 없고, 이 한 줄이 "받기 전에는 다음 알을 쌓을 수 없다"를 보장한다.
            s.box[i].exp = 0
        }
        return egg
    }
