...
struct HatchNotifier: Sendable {
    /// 익은 알 → 알림 문구. 아무것도 안 익었으면 nil. 여러 개면 하나로 묶는다(알림 폭탄 방지).
    /// 이로치는 놓치면 아까운 정보라 문구에서 티가 나야 한다.
    /// **개체가 아니라 알을 받는다** — 개체는 사용자가 확인을 눌러야 생기고, 알림은 그 전에 나간다.
    static func message(for hatched: [Egg], language: AppLanguage) -> (title: String, body: String)? {
    ...
    /// 만들지 않는다.
    func notify(hatched: [Egg], language: AppLanguage) {
