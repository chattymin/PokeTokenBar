...
기여에 관심 가져 주셔서 감사합니다! PokeDexBar는 작은 비상업 팬 프로젝트이며,
...
- 풀 리퀘스트 템플릿을 채워 주세요.
- **UI 변경** (`Sources/PokeDexBar/UI/` 아래 무엇이든)은 PR에 before/after를
...
- **사용량 소스 추가** (새 AI CLI) = `UsageProvider` 프로토콜
  (`Sources/PokeDexBar/Core/UsageProvider.swift`)을 새 타입 하나로 구현하고
  `UsageStore.init`의 기본 `providers:` 배열
  (`Sources/PokeDexBar/Core/UsageStore.swift`)에 등록합니다. 이 두 곳만 손대면
  됩니다.
...
PokeDexBar는 **비공식·비상업 팬 프로젝트**이며 Nintendo, Game Freak,
...
- **포켓몬(또는 다른 제3자) 저작물을 커밋하거나 번들하지 마세요** — 스프라이트,
  아트워크, 오디오, 폰트, 대량 이름/데이터 파일. 포켓몬 종·진화 데이터는 공개
  [PokéAPI](https://pokeapi.co)에서, 스프라이트는
  [Pokémon Showdown](https://play.pokemonshowdown.com/sprites/)에서 **런타임에**
  받아 사용자 기기에 로컬 캐시됩니다; 그대로 유지하세요.
