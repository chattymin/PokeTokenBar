...
- **팝오버 안 세로 `ScrollView` 는 legacy 스크롤러 거터가 폭을 먹는다 — `.scrollIndicators(.never)`.**
  "스크롤 막대 항상 표시"(또는 마우스 연결 + "자동")에서 macOS 는 legacy 스크롤러를 쓰고, SwiftUI 는 그
  몫으로 약 16pt 거터를 콘텐츠 옆에 예약한다. 332pt 팝오버에서 상점·가방·도감 로그·상세·설정 화면만
  카드가 좁아져 헤더/푸터와 우측 끝이 어긋나고 두꺼운 트랙이 붙었다(오버레이 스크롤러는 스크롤 중에만
  떠서 이 문제가 없다). 격리 프로브로 확인: 같은 332pt 행이 기본값에선 거터만큼 좁고 `.never` 에선 전폭.
  **왜 못 걸렀나:** 진화 라인(`EvoLineView`)은 같은 원인(가로 스크롤러의 높이 잠식)으로 이미 `.never` 를
  달았지만 그 수정이 해당 뷰 하나로 끝났고 부류 스윕이 없었다. 기본 오버레이 스타일에선 증상이
  보이지 않아 수동 확인에도 안 걸린다. 가로 `ProviderTabBar` 의 `showsIndicators: false` 는 측정상
  `.never` 와 같다(legacy 에서 높이 24pt, `.hidden` 은 41pt).
  **회귀 가드:** `ScrollIndicatorTests` 가 `Sources/PokeTokenBar/**` 의 모든 `ScrollView` 를 찾아 자기 인자
  (`showsIndicators: false`) 또는 트레일링 클로저 직후 모디파이어 체인의 `.scrollIndicators(.never)` 를
  요구한다. 고정 줄 수 창이 아니라 괄호를 맞춰 체인을 읽으므로 이웃 뷰의 모디파이어로 통과하지 않는다.
  상점·도감 수정과 탭바 인자를 각각 되돌리면 해당 `파일:줄` 로 실패하는 것을 확인했다.
