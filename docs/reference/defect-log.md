...
- **외부 API 식별자를 손으로 적은 상수는 폴백이 가려준다 — 죽은 후보가 증상 없이 남는다.**
  `AppLanguage.apiCodes` 의 `.ja` 가 `["ja-Hrkt", "ja"]` 였는데 PokéAPI 가 내보내는 `language.name` 은
  소문자 `ja-hrkt` 다. 딕셔너리 조회라 첫 후보는 **항상 빗나갔지만**, 종 이름은 뒤 후보 `ja` 가 받아서
  1년 넘게 증상이 없었다. 도감 설명의 버전 이름(`/version`)은 `ja` 가 아예 없고 `ja-hrkt` 뿐이라,
  이 죽은 후보를 밟는 첫 소비자가 나오자마자 일본어 화면에 영어 제목(`Sword`)이 떴다.
  테스트도 못 잡았다 — 픽스처가 **코드와 같은 오타**(`"ja-Hrkt"`)를 키로 썼고, `ja` 도 함께 넣어 둬서
  폴백이 통과시켰다. 외부 문자열을 픽스처에 적을 땐 실제 응답에서 복사해 오고, `A || B` 폴백은
  **B 단독**(A 부재) 케이스를 따로 잠근다(`testJaFallsBackToHrktWhenPlainJaMissing`). 철자 자체는
  기계로 막는다(`testAPICodesAreLowercased`). (2026-08-14, 도감 설명 기능에서 발견.)
...
- **폴백이 있는 로더의 결과를 "요청값"으로 기록하지 마라 — 캐시 판정이 영구히 막힌다.**
  `SpriteLoader.cachedImage(shiny: true)` 는 이로치 캐시가 없으면 *일반* 이미지로 폴백하는데(의도된 동작),
  `SpriteView.init` 이 `loadedShiny` 에 **요청한 값**을 그대로 적었다. 결과: 일반 픽셀을 들고 "이로치
  로드됨"이라 주장 → `needsReload` 가 거짓 → 첫 `.task` 가 통과 → **이로치가 영영 안 불러와진다**.
  증상은 토글이 켜진 뒤 *새로 만들어지는* 뷰에서만 난다(토글 ON 뒤 상세 진입, 필터로 칸이 사라졌다
  다시 생김) — 토글 자체는 기존 뷰의 prop 만 바꾸므로 멀쩡히 동작해서 더 늦게 발견된다.
  규칙: **캐시 시드는 실제로 받은 것을 적는다.** 폴백 여부를 알 수 없으면 폴백 안 하는 조회를 따로 둔다
