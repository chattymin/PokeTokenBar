...
`PlayerStore+FoundEgg.canTakeFoundEgg` 에서 **최종진화 게이트 줄을 통째로 지운다** —
`guard evolutionChoices(individual, line: line).isEmpty else { return false }` 가 그 줄이다.
파트너면 되므로 더 이상 최종형일 필요가 없다. 그리고 판정과 초기화를 `eggProgress` 로 바꾼다:
...
- Modify: `Sources/PokeDexBar/Player/PlayerStore+Hatching.swift` — `makeHatchling(from:at:)` 이 개체를 만드는 자리다(부화 진입점은 `claimHatch(eggID:at:)`)
...
`makeHatchling(from:at:)` 의 개체 생성에 `growthRate: egg.growthRate` 를 넘긴다.
